...
- Not yet: the agent has no identity of its own. `X-Agent-Id` is an assertion, trusted because the
  service is the only caller; Keycloak with least-privilege scopes is a deepening item.
